function varargout = writeNeurophysStudy(varargin)
% WRITENEUROPHYSSTUDY MATLAB code for writeNeurophysStudy.fig
%   function writeNeurophysStudy
%
%   This writeNeurophysStudy GUI is the study specific GUI and last step
%   to creating an Neurophysiological Study experiment. Here you will 
%   choose various options to finalize the setup for your 
%   Neurophysiological Study experiment. For better instructions and 
%   details on each of these options please use the AudExpCreator User's 
%   Guide as this space is limited and cannot adequately provide the 
%   needed explanation. 
%             
...
% Edit the above text to modify the response to help writeNeurophysStudy

% Last Modified by GUIDE v2.5 11-Jun-2017 00:04:06

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @writeNeurophysStudy_OpeningFcn, ...
                   'gui_OutputFcn',  @writeNeurophysStudy_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before writeNeurophysStudy is made visible.
function writeNeurophysStudy_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to writeNeurophysStudy (see VARARGIN)

% Choose default command line output for writeNeurophysStudy
...
baseLOpts{1}='Baseline before every stim';
baseLOpts{2}='Baseline before stim set';
set(handles.popupmenu3,'String',baseLOpts);
...
guidata(hObject, handles);
% UIWAIT makes writeNeurophysStudy wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = writeNeurophysStudy_OutputFcn(hObject, eventdata, handles) 
...
selectedType=content{selected};
handles.baseLOpts = selectedType;
...
try
    nSInfo.stimSOpts = handles.stimSOpts;
catch
    content=get(handles.popupmenu1,'String');
    selected=get(handles.popupmenu1,'Value');
    nSInfo.stimSOpts=content{selected};
end

try
    nSInfo.questOpts = handles.questOpts;
catch
    content=get(handles.popupmenu2,'String');
    selected=get(handles.popupmenu2,'Value');
    nSInfo.questOpts=content{selected};
end

try
    nSInfo.baseLOpts = handles.baseLOpts;
catch
    content=get(handles.popupmenu3,'String');
    selected=get(handles.popupmenu3,'Value');
    nSInfo.baseLOpts=content{selected};
end

try
    nSInfo.breakSOpts = handles.breakSOpts;
catch
    content=get(handles.popupmenu4,'String');
    selected=get(handles.popupmenu4,'Value');
    nSInfo.breakSOpts=content{selected};
end

try
    nSInfo.startSOpts = handles.startSOpts;
catch
    content=get(handles.popupmenu5,'String');
    selected=get(handles.popupmenu5,'Value');
    nSInfo.startSOpts=content{selected};
end

try
    nSInfo.startSExitB = handles.startSExitB;
catch
    content=get(handles.popupmenu6,'String');
    selected=get(handles.popupmenu6,'Value');
    nSInfo.startSExitB=content{selected};
end

try
    nSInfo.endSOpts = handles.endSOpts;
catch
    content=get(handles.popupmenu7,'String');
    selected=get(handles.popupmenu7,'Value');
    nSInfo.endSOpts=content{selected};
end

try
    nSInfo.endSExitB = handles.endSExitB;
catch
    content=get(handles.popupmenu8,'String');
    selected=get(handles.popupmenu8,'Value');
    nSInfo.endSExitB=content{selected};
end

try
    nSInfo.blankSOpts = handles.blankSOpts;
catch
    content=get(handles.popupmenu9,'String');
    selected=get(handles.popupmenu9,'Value');
    nSInfo.blankSOpts=content{selected};
end

try
    nSInfo.forceQExitB = handles.forceQExitB;
catch
    content=get(handles.popupmenu10,'String');
    selected=get(handles.popupmenu10,'Value');
    nSInfo.forceQExitB=content{selected};
end

studyInfo.nSInfo = nSInfo;
...
if strcmp(nSInfo.stimSOpts,'Upload focus image (jpg)')
...
elseif strcmp(nSInfo.stimSOpts,'Focus image for black bg')
    stimSStruct.type = 'image';
    stimSStruct.name = 'fImage.jpg';
...
elseif strcmp(nSInfo.stimSOpts,'Focus image for white bg')
    stimSStruct.type = 'image';
    stimSStruct.name = 'fImage.jpg';
...
elseif strcmp(nSInfo.stimSOpts,'Focus point: small cross')
    stimSStruct.type = 'image';
    stimSStruct.name = 'fImage.jpg';
...
elseif strcmp(nSInfo.stimSOpts,'Focus point: small dot')
    stimSStruct.type = 'image';
    stimSStruct.name = 'fImage.jpg';
...
if strcmp(nSInfo.baseLOpts,'Baseline before every stim');
    bLSettings=1;
elseif strcmp(nSInfo.baseLOpts,'Baseline before stim set');
    normalMessage=sprintf(['How many stims are in each stim set?',...
        ' Please make sure that this reflects your stim array',...
        ' (i.e. there is an even division of stims into sets for recording sessions,',...
        ' presentations, etc.).']);
    yesString=0;
    pause(0.1);
    bLSettings=userRespInterface(studyInfo,normalMessage,yesString);
    drawnow
    
    if isempty(bLSettings)==1
        warningMessage = 'You exit before responding. Try over again.';
        pause(0.1);
        warningInterface(studyInfo,warningMessage);
        drawnow
    ...
    elseif bLSettings==1
        warningMessage = ['"1" stimulus does not consitute a stimulus set. Please go back,'...
            ' and choose the option "Baseline before every stim".'];
        pause(0.1);
        warningInterface(studyInfo,warningMessage);
        drawnow
        return;
    elseif rem(studyInfo.stimPerRec,bLSettings)~=0
        warningMessage = sprintf(['The %d stims in your stim array per recording session was not evenly',...
            ' divided into "%d" sets. Please try over again.'], studyInfo.stimPerRec, bLSettings);
        pause(0.1);
        warningInterface(studyInfo,warningMessage);
        drawnow
        return;
...
if strcmp(nSInfo.questOpts,'No question(s)')
    ...
    studyInfo.bLSettings = bLSettings;
    save studyInfo.mat studyInfo
    
    studyInfo = wBuildStimWavSeq(studyInfo);
    save studyInfo.mat studyInfo
    
    if strcmp(nSInfo.forceQExitB,'Default Esc key')
        fQuitKey = 'ESCAPE';
    else
        fQuitKey = 'End';
    end
    ...
    if strcmp(qOpts,'9 scale')
        nSInfo.ratingOpts='Key press: 1-9 scale';
        rOpts=1;
    elseif strcmp(qOpts,'5 scale')
        nSInfo.ratingOpts='Key press: 1-5 scale';
        rOpts=2;
    elseif strcmp(qOpts,'slider scale')
        nSInfo.ratingOpts='Mouse control: slider scale';
        rOpts=3;
        
        normalMessage = ['Because this is a Neurophysiological study, slider scale responses will be',...
    ...
    if buildQStruct
        if strcmp(nSInfo.questOpts,'Same question(s) for every stim')
        ...
        elseif strcmp(nSInfo.questOpts,'Different question(s) based on groups')
        ...
        elseif strcmp(nSInfo.questOpts,'Different question(s) for every stim')
...
if strcmp(nSInfo.breakSOpts,'Default screen text & exit button')
    breakStruct.text='You are on a break. Press the space bar to continue.';
    breakStruct.exitB='Space';

elseif strcmp(nSInfo.breakSOpts,'Customize screen text only')
...
elseif  strcmp(nSInfo.breakSOpts,'Customize exit button only')
    breakStruct.text='You are on a break. Press the space bar to continue.';
...
elseif strcmp(nSInfo.breakSOpts,'Customize both text & exit button')
    normalMessage=['Please type in your "Break" screen text. The default exit',...
        ' button for the "Break" screen is the "space" bar, so you might want to mention',...
        ' this in your text.'];
    yesString=1;
...
if strcmp(nSInfo.startSOpts,'Default "READY" screen')
...
if strcmp(nSInfo.startSExitB,'Default Enter key')
...
if strcmp(nSInfo.endSOpts,'Default "End of Session" screen')
...
if strcmp(nSInfo.endSExitB,'Default Enter key')
...
if strcmp(nSInfo.blankSOpts,'Fixed stim screen & default settings')
    blankStruct.type=stimSStruct.type;
    blankStruct.name=stimSStruct.name;
    blankStruct.duration=[1 3];
elseif strcmp(nSInfo.blankSOpts,'Fixed stim screen & default settings')
    blankStruct.type=stimSStruct.type;
    blankStruct.name=stimSStruct.name;
    normalMessage=['The default fixed screen duration between noise stim and music stim',...
    ...
    elseif duration(1)==1
        ...
        elseif duration(2)==3
            warningMessage = ['You entered 1 for the first duration and 3 for the second duration',...
                ' which are the default values. Please go back and choose the "Fixed stim screen & default settings" instead.'];
            pause(0.1);
            warningInterface(studyInfo,warningMessage);
            drawnow
            return;
        ...
        normalMessage=['The default fixed screen duration between break and noise stim',...
            ' is 3 sec. What is the time duration (in sec) would you like to use instead?'];
        yesString=0;
...
studyInfo=wNSStimTrial(studyInfo);
save studyInfo.mat studyInfo
...
studyInfo=wNSMainExp(studyInfo);
save studyInfo.mat studyInfo
...
studyInfo.lastComp='writeNeurophysStudy(studyInfo)';
