function studyInfo=wESStimTrial(studyInfo)
%   function studyInfo=wESStimTrial(studyInfo)
%
%   This funtion generates the stimTrial script for EEG Study experiments.
%
...
if strcmp(studyInfo.niReady,'Yes NI Device Ready')
    if strcmp(stimSStruct.type,'image')
        line{1}='function stimTrial(wPtr, wRect, bgColor, imdata, i, pahandle, freq,  wavdata, trigger, s)';
    else
        line{1}='function stimTrial(wPtr, wRect, bgColor, fontColor, i, pahandle, wavdata, freq, trigger, s)';
    end
elseif strcmp(studyInfo.niReady,'Not NI Device Ready')
    ...
    if strcmp(stimSStruct.type,'image')
        line{1}='function stimTrial(wPtr, wRect, bgColor, imdata, i, pahandle, freq,  wavdata, trigger)';
    else
        line{1}='function stimTrial(wPtr, wRect, bgColor, fontColor, i, pahandle, wavdata, freq, trigger)';
    end
end
...
line{dOS+19}='duration = length(wavdata{i})/freq{i};';

if strcmp(studyInfo.niReady,'Yes NI Device Ready')
    line{dOS+21}='t=timer;';
    line{dOS+22}='t.StartDelay = 0;';
    line{dOS+23}='t.StartFcn = {@my_callback_fcn1,s,trigger{i}};';
